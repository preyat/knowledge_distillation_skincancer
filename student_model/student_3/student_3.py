...
def setup_data_loaders(df_train, df_val, input_size):
    ...
    training_set = SkinCancerDataset(df_train, transform=train_preproccesing)
    train_loader = DataLoader(training_set, batch_size=32, shuffle=True, num_workers=32)
    
    validation_set = SkinCancerDataset(df_val, transform=val_preproccesing)
...
for fold, (train_idx, val_idx) in enumerate(skf.split(np.zeros(len(df_original)), df_original['cell_type_idx'])):
    ...
    #train model
    epochs = 1
    model, train_loader, validation_loader = train_and_validate_model_student('student3', df_train_fold, df_val_fold, epochs)
    ...
    torch.save(model.state_dict(), f'{results_folder}/model_fold_{fold}.pth')
