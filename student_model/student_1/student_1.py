...
from training_clean import train_and_validate_model_student1,SkinCancerDataset
...
def setup_data_loaders(df_train, df_val, input_size):
    "Convert dataframes to dataset with preprocessing reference : https://www.kaggle.com/code/ayushraghuwanshi/skin-lesion-classification-acc-90-pytorch-5bb988#Step-3.-Model-training "
...
for fold, (train_idx, val_idx) in enumerate(skf.split(np.zeros(len(df_original)), df_original['cell_type_idx'])):
    ...
    train_loader, val_loader = setup_data_loaders(df_train_fold, df_val_fold, 49)
    ...
    model, train_loader, validation_loader = train_and_validate_model_student1('student1', df_train_fold, df_val_fold, epochs)
